...
public class findClosestElements {
    public static void main(String[] args){
        ...
        System.out.println(x+" 에 가까운 "+k+" 개 추출 <- "+arr);
    ...
    public static List<Integer> mySolution(int[] arr, int k, int x){
        ...
        System.out.println(smallCount+" : "+arr[smallCount]+" / "+bigCount+" : "+arr[bigCount]);
        for(int i=1; i<k; i++){
            double smallValue = Math.abs(x-arr[smallCount]);
            double bigValue = Math.abs(x-arr[bigCount]);
            System.out.println("for "+i+" / "+smallCount+" : "+arr[smallCount]+" - "+smallValue+" / "+bigCount+" : "+arr[bigCount]+" - "+bigValue);
            ...
            System.out.println(result+" / "+smallCount+" ~ "+bigCount);
